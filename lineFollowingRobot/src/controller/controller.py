...
Kp_lat  = 1.0;  Ki_lat  = 0.5;  Kd_lat  = 0.1
Kp_head = 1.2;  Ki_head = 0.1;  Kd_head = 0.05

integral_lat  = 0.0;  prev_lat  = 0.0
integral_head = 0.0;  prev_head = 0.0
DT_CTRL = 0.01
VEL_MAX = 1.0
    ...
    def mainThread(self):
        ...
        try:
            ...
            while(vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime):
                ...
                global integral_lat, prev_lat, integral_head, prev_head

                lat = self.mySignals.lateral_error
                hd  = self.mySignals.heading_error

                integral_lat += lat * DT_CTRL
                d_lat         = (lat - prev_lat) / DT_CTRL
                pid_lat       = Kp_lat * lat + Ki_lat * integral_lat + Kd_lat * d_lat
                prev_lat      = lat

                integral_head += hd * DT_CTRL
                d_head         = (hd - prev_head) / DT_CTRL
                pid_head       = Kp_head * hd + Ki_head * integral_head + Kd_head * d_head
                prev_head      = hd

                vel_cmd = pid_lat + pid_head
                if abs(vel_cmd) > VEL_MAX:
                        integral_lat  -= lat * DT_CTRL
                        integral_head -= hd  * DT_CTRL
                self.mySignals.vel_cmd = max(-VEL_MAX, min(VEL_MAX, vel_cmd))
                # End of user custom code region. Please don't edit beyond this point.
                ...
                vsiCanPythonGateway.setCanPayloadBits(self.packBytes('d', self.mySignals.vel_cmd), 0, 64)
