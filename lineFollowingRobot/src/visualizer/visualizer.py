    ...
    def mainThread(self):
        ...
        try:
            ...
            while(vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime):
                ...
                global max_lateral_error, settle_start_time, settle_time
                global settled, steady_state_errors, log_rows

                t   = vsiCommonPythonApi.getSimulationTimeInNs() * 1e-9
                lat = self.mySignals.lateral_error
                hd  = self.mySignals.heading_error
                xp  = self.mySignals.x_pos
                yp  = self.mySignals.y_pos
                vc  = self.mySignals.vel_cmd

                self.plotter.update_data(t, xp, yp, lat, vc)

                if abs(lat) > max_lateral_error:
                    max_lateral_error = abs(lat)

                if not settled:
                    if abs(lat) < SETTLE_THRESHOLD:
                        if settle_start_time is None:
                            settle_start_time = t
                        elif t - settle_start_time >= SETTLE_DURATION:
                            settle_time = settle_start_time
                            settled     = True
                    else:
                        settle_start_time = None

                if t > 20.0:
                    steady_state_errors.append(abs(lat))

                log_rows.append([round(t,6), round(xp,6), round(yp,6),
                                round(lat,6), round(hd,6), round(vc,6)])
                # End of user custom code region. Please don't edit beyond this point.
                ...
                self.mySignals.heading_error, receivedData = self.unpackBytes('d', receivedData, self.mySignals.heading_error)

                signalNumBytes = 8
                receivedData = vsiCanPythonGateway.recvVariableFromCanPacket(signalNumBytes, 0, 64, 12)
                self.mySignals.x_pos, receivedData = self.unpackBytes('d', receivedData, self.mySignals.x_pos)

                signalNumBytes = 8
                receivedData = vsiCanPythonGateway.recvVariableFromCanPacket(signalNumBytes, 0, 64, 13)
                self.mySignals.y_pos, receivedData = self.unpackBytes('d', receivedData, self.mySignals.y_pos)
